...
export default class CustomizedCanvas extends Component {
  ...
  componentDidMount() {
    if (this.canvasRef && this.canvasRef.current && !this._canvas) {
      ...
      const border = `1px solid ${this.props.theme?.colors?.text || (this.props.colorScheme === 'light' ? '#000' : '#fff')
        }`;
  ...
  onSelect = (event) => {
    ...
    if (!this._allowedTabs.includes(activeTab.toLowerCase())) {
      return;
    }
    ...
    shapes.forEach(
      (shape) => {
        if (
          ...
          && shape.tab === activeTab.toLowerCase()
        ) {
          foundShape = true;
